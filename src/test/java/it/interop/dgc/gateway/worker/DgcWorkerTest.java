...
class DgcWorkerTest {
	...
	void testDownloadWorker() throws Exception {
		...
		Mockito.when(restApiClient.downloadTrustListFilteredByType(CertificateType.UPLOAD)).thenReturn(emptyUploadCerts);
	...
	void testDownloadWorker_notDeleteUKCerts() throws Exception {
		...
		queryUKDSC.addCriteria(Criteria.where("raw_data").is(DgcWorkerTestHelper.UK_DSC_TO_DOWNLOAD_RAW_DATA));
		queryUKDSC.addCriteria(Criteria.where("certificate_type").is(CertificateType.DSC));
		queryUKDSC.addCriteria(Criteria.where("revoked").is(false));
		SignerInformationEntity UKDSCDownloaded = mongoTemplate.findOne(queryUKDSC, SignerInformationEntity.class);
		...
		Assertions.assertEquals("MANUALLY", UKDSCDownloaded.getDownloadBatchTag());
	}
	...
	void testDownloadWorker_getCountryList() throws Exception {
		...
		Mockito.when(restApiClient.downloadTrustList()).thenReturn(emptyTrustList);
		
		RestApiResponse<String> countryListResponse = DgcWorkerTestHelper.getCountryList();
		Mockito.when(restApiClient.downloadCountryList()).thenReturn(countryListResponse);
		...
		queryCountryList.addCriteria(Criteria.where("raw_data").is(DgcWorkerTestHelper.COUNTRY_LIST));
		CountryListEntity countryList = mongoTemplate.findOne(queryCountryList, CountryListEntity.class);

		String batchTag = countryList.getDownloadBatchTag();
		
		Assertions.assertNotNull(countryList);
		Assertions.assertNotNull(batchTag);
		Assertions.assertEquals(countryList.getHash(), DgcWorkerTestHelper.COUNTRY_LIST_HASH);
	
		Query queryLog = new Query();
		queryLog.addCriteria(Criteria.where("batch_tag").is(batchTag));
		queryLog.addCriteria(Criteria.where("item_type").is(DgcRuleLogEntity.ItemType.COUNTRIES));
		DgcRuleLogEntity log = mongoTemplate.findOne(queryLog, DgcRuleLogEntity.class);
		Assertions.assertEquals(DgcRuleLogEntity.OperationType.DOWNLOAD, log.getOperation());
		Assertions.assertEquals("200 OK", log.getExecutionReport());
	}
	...
	void testDownloadWorker_getValuesets() throws Exception {
		...
		for(int i = 0; i < valueSetsIds.getData().size(); i++) {
			Mockito.when(restApiClient.getValueset(valueSetsIds.getData().get(i))).thenReturn(new RestApiResponse<String>(HttpStatus.OK, null, valueSetValues.get(i)));
		}
		...
		queryValuesets.addCriteria(Criteria.where("hash").is(DgcWorkerTestHelper.VALUESET_1_HASH));
		ValueSetEntity valueset = mongoTemplate.findOne(queryValuesets, ValueSetEntity.class);
		
		Assertions.assertNotNull(valueset);
		String batchTag = valueset.getDownloadBatchTag();
		Assertions.assertNotNull(batchTag);
		Assertions.assertEquals(valueset.getRawData(), DgcWorkerTestHelper.VALUESET_1);
	
		Query queryValuesets2 = new Query();
		queryValuesets2.addCriteria(Criteria.where("hash").is(DgcWorkerTestHelper.VALUESET_2_HASH));
		ValueSetEntity valueset2 = mongoTemplate.findOne(queryValuesets2, ValueSetEntity.class);
		...
		Assertions.assertEquals(valueset2.getRawData(), DgcWorkerTestHelper.VALUESET_2);
	
		Query queryValuesets3 = new Query();
		queryValuesets3.addCriteria(Criteria.where("hash").is(DgcWorkerTestHelper.VALUESET_3_HASH));
		ValueSetEntity valueset3 = mongoTemplate.findOne(queryValuesets3, ValueSetEntity.class);
		...
		Assertions.assertEquals(valueset3.getRawData(), DgcWorkerTestHelper.VALUESET_3);
		
		Query queryLog = new Query();
		queryLog.addCriteria(Criteria.where("batch_tag").is(batchTag));
		queryLog.addCriteria(Criteria.where("item_type").is(DgcRuleLogEntity.ItemType.VALUES));
		DgcRuleLogEntity log = mongoTemplate.findOne(queryLog, DgcRuleLogEntity.class);
		Assertions.assertEquals(DgcRuleLogEntity.OperationType.DOWNLOAD, log.getOperation());
		Assertions.assertEquals("200 OK", log.getExecutionReport());
	}
	...
	void testDownloadWorker_getBusinessRules() throws Exception {
		...
		Mockito.when(restApiClient.downloadTrustListFilteredByType(CertificateType.UPLOAD)).thenReturn(uploadCerts);
		
		RestApiResponse<Map<String, List<ValidationRuleDto>>> ruleResp = DgcWorkerTestHelper.getRulesResponse();
		Mockito.when(restApiClient.downloadValidationRules("EU")).thenReturn(ruleResp);
		
		Map<String, List<TrustListItemDto>> mapCountryTrust = new HashMap<>();
		uploadCerts.getData().forEach(trust -> mapCountryTrust.computeIfAbsent(trust.getCountry(), k -> new ArrayList<>()).add(trust));
		
		Mockito.when(signatureVerifier.checkRuleUploadCertificate(ruleResp.getData().get("GR-EU-0001").get(0), mapCountryTrust.get("EU"), "EU")).thenReturn(true);
		Mockito.when(signatureVerifier.map(ruleResp.getData().get("GR-EU-0001").get(0))).thenReturn(DgcWorkerTestHelper.getDummyValidationRule());
		worker.downloadWorker();

		List<BusinessRuleEntity> businessRules = mongoTemplate.findAll(BusinessRuleEntity.class);
		...
		List<BusinessRuleInvalidEntity> businessRulesInvalid = mongoTemplate.findAll(BusinessRuleInvalidEntity.class);
		Assertions.assertNotNull(businessRulesInvalid);
		Assertions.assertEquals(businessRulesInvalid.size(), 0);
		
		Query queryLog = new Query();
		queryLog.addCriteria(Criteria.where("batch_tag").is(batchTag));
		queryLog.addCriteria(Criteria.where("item_type").is(DgcRuleLogEntity.ItemType.RULES));
		DgcRuleLogEntity log = mongoTemplate.findOne(queryLog, DgcRuleLogEntity.class);
		Assertions.assertEquals(DgcRuleLogEntity.OperationType.DOWNLOAD, log.getOperation());
		Assertions.assertEquals("200 OK", log.getExecutionReport());
	}
	...
	void testDownloadWorker_getBusinessRulesInvalid() throws Exception {
		...
		BusinessRuleInvalidEntity businessRuleInvalid = businessRulesInvalid.get(0);
	...
	void testUploadWorker_uploadNewBusinessRule() throws Exception {
		...
		String base64RawData = Base64.getEncoder().encodeToString(DgcWorkerTestHelper.RULE_TO_UPLOAD.getBytes());
		Mockito.when(signatureService.getSignatureForBytes(base64RawData))
				.thenReturn(DgcWorkerTestHelper.SIGNATURE_SERVICE_MOCKDATA);
		Mockito.when(
				restApiClient.uploadValidationRule(DgcWorkerTestHelper.SIGNATURE_SERVICE_MOCKDATA, "IT"))
				.thenReturn(new RestApiResponse<String>(HttpStatus.CREATED, null, "201 CREATED"));
		...
		Query query = new Query();
		query.addCriteria(Criteria.where("raw_data").is(DgcWorkerTestHelper.RULE_TO_UPLOAD));
		BusinessRuleUploadEntity uploadedCert = mongoTemplate.findOne(query, BusinessRuleUploadEntity.class);
		Assertions.assertNotNull(uploadedCert.getUploadBatchTag());
		...
		queryLog.addCriteria(Criteria.where("item_type").is(DgcRuleLogEntity.ItemType.RULES));
		DgcRuleLogEntity log = mongoTemplate.findOne(queryLog, DgcRuleLogEntity.class);
		Assertions.assertEquals(DgcRuleLogEntity.OperationType.UPLOAD, log.getOperation());
		Assertions.assertEquals("201 CREATED", log.getExecutionReport());
	}
	...
	void testUploadWorker_revokeExistingBusinessRule() throws Exception {
		...
		String base64RawData = Base64.getEncoder().encodeToString("GR-IT-0001".getBytes());
		Mockito.when(signatureService.getSignatureForBytes(base64RawData))
				.thenReturn(DgcWorkerTestHelper.SIGNATURE_SERVICE_MOCKDATA);
		Mockito.when(
				restApiClient.deleteValidationRules(DgcWorkerTestHelper.SIGNATURE_SERVICE_MOCKDATA, "IT"))
				.thenReturn(new RestApiResponse<String>(HttpStatus.NO_CONTENT, null, "204 NO CONTENT"));
		...
		Query queryLog = new Query();
		queryLog.addCriteria(Criteria.where("batch_tag").is(uploadedCert.getRevokedBatchTag()));
		queryLog.addCriteria(Criteria.where("item_type").is(DgcRuleLogEntity.ItemType.RULES));
		DgcRuleLogEntity log = mongoTemplate.findOne(queryLog, DgcRuleLogEntity.class);
		Assertions.assertEquals(DgcRuleLogEntity.OperationType.REVOKE, log.getOperation());
		Assertions.assertEquals("204 NO_CONTENT", log.getExecutionReport());
	}
